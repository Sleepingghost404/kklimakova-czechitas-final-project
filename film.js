...
if (film) {
  ...
  const { url, sirka, vyska } = film.plakat;

  Object.assign(detailContainer.querySelector('img'), {
    src: url,
  ...
  detailContainer.querySelector('.card-title').textContent = film.nazev;
  detailContainer.querySelector('.card-text').textContent = film.popis;

  const premiere = dayjs(film.premiera);
  const diff = premiere.diff(dayjs(), 'days');
  const daysText = diff > 0
    ? `za ${diff} ${diff === 1 ? 'den' : diff < 5 ? 'dny' : 'dní'}`
    : diff < 0
    ? `před ${Math.abs(diff)} ${Math.abs(diff) === 1 ? 'dnem' : 'dny'}`
    : 'dnes';

  detailContainer.querySelector('#premiera').innerHTML = `Premiéra <strong>${premiere.format('D. M. YYYY')}</strong>, což ${daysText}.`;
} else {
  alert('Film nenalezen.');
...
document.querySelectorAll('.fa-star').forEach((star, index) => {
  ...
  star.addEventListener('mouseleave', () => highlightStars(lastClickedRating));
});
...
if (noteForm) {
  noteForm.addEventListener('submit', e => {
    ...
    [messageInput, termsCheckbox].forEach(el => el.classList.remove('is-invalid'));
    ...
    note.textContent = messageInput.value;
...
document.addEventListener('keydown', e => {
  if (e.code === 'Space' && !['TEXTAREA', 'INPUT', 'BUTTON'].includes(e.target.tagName)) {
    e.preventDefault();
